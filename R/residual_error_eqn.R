...
get_residual_error_eqn <- function(elements, ipred, .type) {
  ...
  if (!all(elements %in% SUPPORTED_ELEMENTS)) {
    stop(glue("residual error eqn mapping only supports:
              {elements}",
              elements = paste0(SUPPORTED_ELEMENTS, collapse = '/')))
  }
  ...
  if (length(elements) == 1) {
    return(switch(elements,
      ADD = glue("{ipred} + {.nm}", .nm = name_mapping[1]),
      PROP = glue("{ipred}*(1+{ipred}*{.nm})", .nm = name_mapping[1])
    ))
  }
  prop <- ifelse(.type == "nonmem", glue("EPS({eps_num})",
                                        eps_num = which(elements == 'PROP')
                                        ), "PROP")
  add <- ifelse(.type == "nonmem", glue("EPS({eps_num})",
                                       eps_num = which(elements == 'ADD')
                                       ), "ADD")
